...
void ImGuiCommentBox::onInspect(GraphView* inspector)
{
  ...
  if (ImGui::InputTextMultiline("##Comment", text_.data(), text_.capacity(), ImVec2(0, 0), ImGuiInputTextFlags_EnterReturnsTrue))
...
bool CommandManager::SimpleCommand::onUpdatePrompt(GraphView* view)
{
  ...
  bool confirmed =
    ImGui::InputText("##prompt", promptInput_.data(), ImGuiInputTextFlags_EnterReturnsTrue);
  if (confirmed && view) {
      ...
      [&newviews, &dockIdMap](DockLayoutNode const* parent, DockLayoutNode const& current) {
        ...
        if (auto itr = dockIdMap.find(&current); itr != dockIdMap.end()) {
          ...
          if (current.split == 0) {
            ...
            for (size_t i=2, n=current.children.size(); i<n; ++i) {
              ImGuiID next;
...
bool CreateNodeState::update(NetworkView* view)
{
  ...
  if (!isConfirmed_) {
    ...
    if (ImGui::BeginPopup("CreateNode")) {
      ...
      isConfirmed_ =
        ImGui::InputText("##nodeClass", newInput.data(), ImGuiInputTextFlags_EnterReturnsTrue);
      ImGui::Separator();
