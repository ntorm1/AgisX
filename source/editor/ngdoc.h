...
#define DEFINE_MSG_VARIANT_(func, cat, verb)                                        \
  ...
  template<class... T>                                                              \
  static inline void func##f(const char* runtime_format_string, T... args)                 \
  {                                                                                 \
    EMIT_MESSAGE_(std::vformat(runtime_format_string,  std::make_format_args(std::forward<T>(args)...)), cat, verb);               \
  }
...
#undef EMIT_MESSAGE_
    ...
    public:
        ...
        virtual bool serialize(nlohmann::json& json) const override;
        virtual bool deserialize(nlohmann::json const& json) override;
    ...
    public:
        ...
        virtual Vector<Vec2> calculatePath(
            ...
            AABB startBound = { {0, 0} },
            AABB endBound = { {0, 0} });
        virtual bool serialize(nlohmann::json& json) const;
        virtual bool deserialize(nlohmann::json const& json);
    };
    // }}} Graph
